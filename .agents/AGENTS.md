...
> ## 🚧 지금 진행 중인 일 (2026-08-31 갱신)
>
> ~~협의회 교사 프리셋이 `feature/meeting-presets`에 대기 중~~ — **완료.** 운영 DB에
> `20260821120000_add_meeting_preset`이 적용되었고, `main`에 합쳐져 이후 커밋들(이수증 프리셋
> 엑셀 가져오기, 쌤스 헬퍼 디자인 전면 개편)까지 문제없이 얹혔습니다. 브랜치는 다 합쳐졌으므로
> 로컬·원격 모두 지워도 안전합니다(2026-08-31에 로컬은 정리함, 원격 `origin/feature/meeting-presets`는
> 아직 GitHub에 남아 있으니 필요하면 지우세요).
>
