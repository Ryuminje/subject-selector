...
- **교체와 보강은 문서에서 줄 수가 다릅니다 — 이게 이 기능의 핵심입니다.** 교체는 서로 시간을 맞바꾸므로 **두 줄**(① 내 수업을 상대가 ② 상대 수업을 내가)이 나가고, 보강은 상대가 대신 들어가는 것뿐이라 **한 줄**입니다. 두 번째 줄에서 `fromTeacher`/`toTeacher`가 뒤집히는 게 중요합니다 — 안 뒤집으면 "그럼 저 선생님 수업은 누가 하냐"가 되어 결재에서 반려됩니다. 이 분해는 `lib/makeup/buildRows.ts`의 `rowsForEntry()` 한 곳에서만 합니다.
...
- **서버에 저장하지 않습니다.** 사용자가 1단계 범위를 "저장 없음"으로 정했습니다. 그래서 인쇄 페이지로는 **sessionStorage**(`MAKEUP_DOC_KEY`)로 넘깁니다 — 나중에 "내가 낸 보강원 목록/교무부장 전체 조회"를 열려면 그때 테이블을 추가하면 됩니다.
- **⚠️ 문서 모양은 `src/app/apps/schedule-helper/makeup/print/page.tsx` 한 파일에만 있습니다.** 학교 한글(.hwp) 서식을 받으면 그 파일의 `<article>` 안 표와 머리말만 바꾸면 되고 담기·날짜 계산·데이터 전달은 손댈 필요가 없습니다. **한글 파일 자체를 채워 내보내는 건 불가능**하므로(AI 파트너가 hwp 업로드를 막아둔 것과 같은 이유) 같은 모양을 화면으로 재현해 인쇄·PDF로 뽑는 방식입니다. 현재 표는 서식을 받기 전 임시 형태입니다.
...
- 설계·구현 내용은 위 "📝 보강원 작성(makeup) 참고 메모" 섹션에 정리했습니다. 특히 **교체가 두 줄로 펼쳐지는 이유**와 **요일→날짜 계산**은 다음에 손댈 때 반드시 먼저 읽으세요.
