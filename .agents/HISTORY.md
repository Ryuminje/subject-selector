...
- **대신 입력 단계에서 오타를 잡음(사용자 제안).** 3단계 시간표 입력 과목명 칸에 2단계 학생 데이터 과목명 `<datalist>` 자동완성 + 데이터에 없는 이름은 빨간 칸 + "N칸" 경고. 5단계 변경전/변경후 칸도 같은 자동완성(크롬은 부분 문자열로 후보 표시) + 빨간 표시. 5단계 후보는 **학생 데이터 과목 ∪ 시간표에 열린 과목**(`changeSubjectOptionsOf`) — 처음엔 학생 데이터만 썼다가 현재 수강생 0명인 과목(변경 목적지로 흔함)이 빨갛게 나오는 걸 검증 중 발견해 넓힘. 3단계 시간표 검사는 학생 데이터 기준 유지. 참고: 실제 데이터에서 `논술`, `체육 + 진로` 같은 반 고정 과목이 2단계 선택과목 파일에 없어 빨갛게 뜸(사용자에게 제외 여부 물어둠, 미결).
