...
**운영 DB 마이그레이션 미적용 복구 + 쌤스 헬퍼 "이미 이뤄진 교체 기록" 날짜 수정·월별 보기:**
- **복구.** 다른 컴퓨터에서 `School.manualChanges`를 쓰는 코드(교체 기록)가 푸시돼 Vercel에 배포됐는데 운영 DB엔 마이그레이션 3건(`20260918120000_add_teacher_date_blocks`, `…140000_drop_teacher_date_blocks`, `…150000_add_school_manual_changes`)이 미적용이라 시간표 조회가 실패. 진단 중 알게 된 것: NAS의 `my-webapp-subject-selector-1` 컨테이너는 2달 전 생성 그대로(최근 `deploy.sh`로 재생성된 적 없음)라 **그 안에서 `prisma migrate status`를 돌리면 옛 마이그레이션 폴더 기준으로 "최신"이라고 잘못 나올 수 있음** — 새 마이그레이션이 있는 개발 PC 저장소에서 `.env`(NAS DB `192.168.0.21:55432`)로 돌려야 정확. NAS에서 `sudo`는 비밀번호가 필요하지만 `docker compose` 자체는 sudo 없이 됨. 적용은 규칙대로 사용자가 개발 PC에서 직접 `npx prisma migrate deploy` 실행 → 3건 적용, 시간표·교체 기록 막대 정상 확인. `git pull` 뒤 로컬에서 `npx prisma generate`를 안 하면 `tsc`가 `manualChanges` 필드를 모른다고 실패함.
- **교체 기록 날짜 수정.** 사용자 요청: "해당 주가 아닌 다른 날짜일 수도 있어서 날짜를 수정할 수 있도록". 기록 목록 각 줄의 결강일/교체일을 `<input type=date>`로 바로 고침 → `PATCH /api/schedule-helper/manual-changes`(신규). 시간표가 요일 단위라 **칸의 요일과 다른 날짜는 서버에서 거부**("월요일 수업이라 월요일 날짜만…") — 그렇게 저장되면 `applyManualChanges`/`affectsWeek`가 어느 주에도 매칭하지 못해 조용히 무효가 되기 때문. 기록 방식(두 칸 클릭 → 기준일 주로 저장)은 그대로고, 저장 후 목록에서 날짜를 옮기는 흐름.
- **월별 보기.** 사용자 요청으로 목록 위에 「이번 주」(기본) + 기록이 있는 달별 칩(「9월 3」). 달을 고르면 결강일·교체일 중 하나라도 그 달인 기록을 날짜순으로 표시(두 달에 걸친 교체는 양쪽에). 중간에 넣었던 "다른 주 기록 N건 보기" 토글은 이걸로 대체. 서버가 마지막 날짜 기준 60일 지난 기록을 정리(`KEEP_DAYS`)하므로 지난 달은 약 두 달 전까지만 보임 — 화면에 안내.
- 검증: `tsc` 클린. 로컬 dev 서버도 **운영 DB에 붙어 있어** 실제 `PATCH` 호출은 하지 않음(학교 공유 기록이 바뀜). 로그인 밖 임시 페이지(`src/app/dev-preview-manual`, 확인 후 삭제)에 가짜 기록을 넣어 요일 불일치 거부, 다른 주로 옮기기, 월 칩 필터·건수·두 달 걸친 기록을 DOM으로 확인. 서버 PATCH의 요일 검사는 같은 규칙(`weekdayOf`)을 쓰지만 실제 호출로는 미검증 — 배포 후 기록 하나로 확인 요청.
- 참고(사용자 질문): 김경희 화3(1-5) ↔ 류민제 수4(1-8) 교체가 안 뜬 건 교체 기록 때문이 아니라 **다른 반**이라서 — 교체는 같은 학년·반 수업끼리만 후보(`SwapTab.tsx`의 swap 루프).

### 2026-09-16

**선택과목 변경(change-survey) 탭 — 과목명 완전 일치, 입력 자동완성·불일치 표시, 3단계 교체, 시간표 지우기, 확정 내역 삭제, 학년 간 결과 섞임 수정:**
- **과목명 매칭을 "포함하면 같은 과목" → 완전 일치로.** 사용자 제보: 같은 타임의 `심화 영어`와 `심화 영어 독해와 작문` 수강생이 명단에서 한쪽으로 합쳐지고, 과목 필터에서는 양쪽에 다 뜸. 원인은 `RosterStep`/`RosterAfterStep`/`useChangeExports`/`useElectiveChanges`/`lib/subjectMatch`에 흩어진 양방향 `includes` 비교. 전체 보기에서는 `.find()`가 먼저 걸린 `심화 영어`로 몰아넣고, 필터에서는 화면에 남은 칸 하나가 포함 조건에 걸려 섞였음. 더 위험하게는 자동 변경 배정(`subjectExistsInSlot`)이 독해와작문만 열린 타임을 "심화 영어 개설"로 오판할 수 있었음. `lib/subjectMatch.ts`의 `normalizeSubject`(공백·로마 숫자·대소문자·**끝** 분반 번호만 흡수) + `sameSubject`로 전부 통일. `useChangeExports`는 예전에 `[ⅠⅡⅢⅣ1234]`를 이름 **어디서든** 지워 `물리학Ⅰ`/`물리학Ⅱ`가 같아지는 문제도 있었는데 함께 사라짐. `useElectiveChanges.ts` 안에 있던 `normalizeSubject` 복사본도 제거.
- **대신 입력 단계에서 오타를 잡음(사용자 제안).** 3단계 시간표 입력 과목명 칸에 2단계 학생 데이터 과목명 `<datalist>` 자동완성 + 데이터에 없는 이름은 빨간 칸 + "N칸" 경고. 5단계 변경전/변경후 칸도 같은 자동완성(크롬은 부분 문자열로 후보 표시) + 빨간 표시. 5단계 후보는 **학생 데이터 과목 ∪ 시간표에 열린 과목**(`changeSubjectOptionsOf`) — 처음엔 학생 데이터만 썼다가 현재 수강생 0명인 과목(변경 목적지로 흔함)이 빨갛게 나오는 걸 검증 중 발견해 넓힘. 3단계 시간표 검사는 학생 데이터 기준 유지. 참고: 실제 데이터에서 `논술`, `체육 + 진로` 같은 반 고정 과목이 2단계 선택과목 파일에 없어 빨갛게 뜸(사용자 결정: 반 고정 과목도 그대로 빨간색으로 둠).
- **3단계 교체 추가.** 기존은 1단계(같은 타임 직접)·2단계(Y타임 과목을 X타임으로) 까지. 2단계 후보가 하나도 없을 때만 3단계: X←Z타임 과목, Y←변경후 과목, Z←Y타임 과목. 순차/최적화(DFS) 두 경로 모두, 최적화는 세 반 인원의 최대값으로 비용 비교. 로그 3줄에 `chain: 3`, 5단계 결과 내역에 보라색 "3단계" 배지. 로그 순서(옮겨지는 과목 먼저, 신청 과목 마지막)를 리로스쿨 파일/명단이 타임별로 재생하는 방식이 전제로 함 — 코드 추적으로 맞음 확인, 6단계 명단은 브라우저로 확인.
- **3단계 시간표 행/열/전체 내용 지우기**(`useTimetableData.clearTimetable`) — 칸 내용만 비우고 타임·반 구조는 유지(기존 휴지통은 구조 삭제). 확인창 있음.
- **5단계 "확정 내역 삭제"**(`handleClearConfirmed`) — 현재 학년의 `confirmedLog`/`confirmedBaseSchedules`와 **`confirmHistory`까지** 비움(안 비우면 나중 "확정 취소"가 지운 내역을 되살림). 미확정 신청 표와 다른 학년은 유지. 저장 파일에서도 없애려면 다시 저장해야 함.
- **학년 간 결과 섞임(실측 버그).** 3학년에서 등록한 신청 결과가 2학년 탭에도 뜸. `pendingResult.log`와 `adjustmentLog`가 두 학년을 학번 하나의 맵으로 합쳤고, 사용자의 2학년 명단에도 같은 학번(20402)이 있어서 필터(`gradeStudentIds`)를 통과함. 5단계뿐 아니라 6단계 명단·7단계 분석·엑셀·리로스쿨 파일에도 섞일 수 있는 구조였음. 로그를 학년별(`adjustmentLogByGrade`)로 분리하고 `adjustmentLog`는 현재 학년만; `useChangeExports`는 학년별 맵을 받아 각 핸들러의 학년(리로스쿨은 인자로 받은 학년)으로 꺼냄. 두 학년 명단이 같은 학번을 갖는 것 자체는 2단계 업로드 파일 문제일 가능성이 커서 사용자에게 확인 요청.
- 검증: 매 단계 `tsc --noEmit` 클린. 매칭 규칙은 일회용 assertion 스크립트(삭제함), 나머지는 로컬 브라우저에서 `loadChangeBackup`으로 합성 데이터를 주입해 명단/필터/빨간 표시/1·2·3단계·실패 케이스(순차·최적화 둘 다)/6단계 반영/지우기/확정 삭제/학년 분리를 DOM으로 확인 후 새로고침으로 정리. 엑셀·리로스쿨 파일 실제 다운로드는 안 해봄. datalist 드롭다운은 네이티브 팝업이라 캡처 불가.

### 2026-09-13

**타임 배정 결과 → 리로스쿨 업로드용 엑셀 내보내기 추가:**
- 사용자 요청: "본조사 데이터와 똑같은 데이터에서 1이라 표현되어 있던 숫자를 현재 배정된 A, B, C, D와 같은 타임을 넣어" 받고 싶다. 즉 새 표가 아니라 **원본 파일 그 자체의 매트릭스 셀만 바꾼 파일**이 필요합니다(리로스쿨에 도로 올려야 하므로 열 구성·메타 열·지정과목 열이 원본과 일치해야 함).
- 그래서 `lib/exportRiroschool.ts`(신규)는 본조사 탭이 들고 있는 **원본 업로드 파일을 다시 열어** 셀 값만 덮어씁니다. 원본 바이트는 이미 `MainSurveyTab`의 `getMainBackup().uploadedFiles[학년].data`에 data URL로 보관돼 있었고(`useMainUploads.ts`가 `readAsDataURL`로 저장), `MainSurveySnapshot`에 그 필드만 추가해 타임 배정 탭에서 쓰게 했습니다. 같은 방식이 선택과목 변경 탭 `useChangeExports.ts`의 `handleDownloadRiroschool`에 이미 있었으므로 그 패턴을 따랐습니다(그쪽은 변경조사 데이터 전용이라 재사용은 불가).
- 시트 해석 규칙은 본조사 파서(`useMainUploads.ts`)와 **같은 가정을 그대로** 씁니다 — 학번+이름이 함께 있는 행을 헤더로 탐지, 2줄 병합 헤더면 상위 헤더를 forward-fill로 복원, 과목 매트릭스는 H열(인덱스 7)부터, 선택 표시 값은 `1/y/o/이수/참여`, `지정` 들어간 헤더는 제외. 과목 매칭 키는 `fromMainSurvey.ts`와 같은 `정규화(학기)|정규화(과목명)`(실패 시 이름만으로 폴백). 배정이 없는(미배정) 칸은 원본 값을 그대로 둡니다 — 지우면 신청 사실까지 사라지므로.
- **검증 2단계**: ① 실제 본조사 샘플 `samples/sample6.xlsx`로 노드 라운드트립(임시 스크립트, 실행 후 삭제) — 타임 문자 1120칸 전부 기대값 일치, 미배정 224칸 원본 유지, 메타 열 변형 0. 이때 **sample6의 첫 데이터 행이 학생이 아니라 집계 행(198/31/103)**이라 "데이터 시작행 + i = 학생 i의 행"이라는 검증 스크립트 쪽 가정이 깨졌었음(로직 버그는 아니었고, 집계 행은 학번이 로스터에 없어 자연히 안 건드려짐). ② 로컬 브라우저에서 실제 경로 전체(본조사 업로드 → 타임배정 연동 → 배정 최적화 → 버튼) 실행: "학생 224명 · 1220칸 채움 · 미배정 124칸은 원본 값 유지", 콘솔 에러 없음.
- `tsc --noEmit` 클린.

### 2026-09-12 (2)

**반 고정 공통과목 spread(최대 분산) 배치가 선택과목 쪽에 미배정을 유발 — 자동 spread→pack 폴백 추가:**
- 같은 날 앞선 항목에서 반 고정 공통과목을 "가능한 한 여러 타임에 나눠(spread)" 배치하도록 바꿨는데, 사용자가 실제로 써보니 "최적화 완료 (2557회 탐색, 미배정 5명)"처럼 선택과목 쪽에 미배정이 생기는 걸 발견. 원인 추정: spread는 교사 수 상한 안에서 되도록 많은 서로 다른 타임을 건드리는데, 그러면 그리드 전체의 "어느 반도 안 막힌" 완전히 빈 타임이 줄어들어 선택과목 배정(`pickTimes`/`runAssign`)이 그만큼 빡빡해짐(선택과목 배치 자체는 반고정공통 배치를 전혀 모른 채 독립적으로 결정되고, 이후 `blockedBands`로 학생별 매칭만 걸러내는 구조라 우연히 겹치면 못 뺌).
- 사용자 요청: "최대한 펼치되, 미배정이 나오면 알아서 뭉쳐도 됨(교사 수 상한만 안 넘으면 됨)." `SemesterSettings.bandMode`(`"spread"`|`"pack"`, 기본 spread) 추가하고, `computeFixedBands`에 `mode` 파라미터 신설 — pack 모드는 같은 그리디 반복문에서 후보 타임 정렬 기준만 뒤집어(이미 이 구획이 들어간 타임을 먼저 채움) 기존 "타임을 최소화해 교사 수까지 꽉 채우는" 동작을 재현. `useTimeAllocation.ts`의 `runOptimize`가 이제 spread로 먼저 배정 최적화를 돌려보고, 미배정>0이면 그 미배정을 유발한 학기만 pack 모드로 bandTimes를 다시 계산해 통째로 재최적화, **실제로 미배정이 줄었을 때만** 그 결과와 `bandMode:"pack"` 설정을 채택(패치해서 그리드 표시와도 일관되게 유지)합니다 — 비교 기반이라 손해 볼 일은 없는 구조.
- **검증 한계**: 로직은 `tsc` 클린이고 "두 방식을 실제로 돌려 비교해 나은 쪽만 채택"이라 구조적으로 안전하지만, 사용자가 겪은 정확한 미배정 유발 조합(225명 데이터, 특정 교사 수 배치)을 합성 데이터로 재현하는 덴 실패(간단한 합성 시나리오에선 spread/pack 둘 다 0명으로 나와 차이가 안 드러남 — 조건 맞추기 까다로움). 실제 데이터로 재검증 필요하다고 사용자에게 명시적으로 알림.
...
- **이 저장소 밖(임시 작업폴더)에서 처음부터 만든 것을 옮겨온 것입니다.** 사용자가 학교에서 쓰던 기존 도구의 화면을 캡처해 보여주며 "이런 걸 만들어 달라"고 해서 의존성 없는 순수 HTML/CSS/JS로 구현했고, 완성 후 "원래 작업하던 채팅에서 이어서 하고 싶다"고 해서 이 저장소에 넣고 문서를 남겼습니다. **아직 `src/`에 연결되지 않았고 빌드에도 안 들어갑니다.** 상세 문서는 `prototypes/time-allocation/README.md`.
- **하는 일**: 수강신청 결과(엑셀에서 붙여넣기)를 받아 과목별 분반을 타임에 배치하고 학생을 겹치지 않게 배정. 학생 한 명당 "선택 과목 ↔ 타임" 이분 매칭을 돌리고, 분반 위치를 무작위로 바꿔가며 `미배정×1000 + 정원초과×20 + 불균형×0.05` 비용을 낮추는 언덕오르기로 최적화합니다. 초기 배치는 과목 간 동시 선택 빈도를 써서 같이 선택되는 과목을 서로 다른 타임에 놓습니다.
- **요구사항이 대화 중에 세 번 커졌고, 그때마다 구조를 일반화했습니다.** ① 처음엔 3학점 선택과목만 → ② "모든 학생이 듣는 3·2·1학점 과목도 넣어달라, 1학점은 자기 반에서" → ③ "체육 교사가 2명뿐인데 8개 반이 되나?" → ④ "공통과목을 내가 임의로 추가·삭제하고 학점도 지정하게 해달라". ②에서 `2학점+1학점 = 3학점 = 한 타임`이라는 **구획(band)** 개념이 나왔고, ③에서 교사 수 제약(`한 타임에 최대 n개 반 → ceil(반 수/n)개 타임에 분산`)이 들어갔으며, ④에서 하드코딩된 3개 슬롯을 과목 리스트 + 구획 자동 패킹(first-fit decreasing)으로 갈아엎었습니다. **처음부터 구획으로 설계했으면 ②~④가 전부 같은 구조였을 것** — 학점이 다른 과목을 "타임을 시수로 나눠 쓰는 묶음"으로 보는 게 이 도메인의 올바른 추상화입니다.
- **실측으로 잡은 버그 1 — 사용자 데이터의 열 누락.** 사용자가 처음 붙여넣은 자료로 돌렸더니 학생마다 한 타임씩 비었습니다. 합계 행을 검산해 2학기 합이 1383(기대 1520)이고 차이 137이 `과학의 역사와 문화` 인원과 정확히 일치함을 보여 **프로그램이 아니라 엑셀 숨겨진 열 문제**임을 특정했습니다. 이후 불러올 때 각 학생의 `과목수` 열과 실제 파싱된 선택 수를 비교해 경고하는 검증을 넣었습니다(열 하나를 지운 샘플로 경고 동작 확인). **이식할 때 이 검증을 빼지 마세요.**
- **실측으로 잡은 버그 2 — 같은 구획 과목의 이중 계산.** 사용자가 "체육 2 + 진로 1인데 인원이 부풀려져 나온다"고 지적. 확인해 보니 진로가 체육과 같은 구획을 가리키는데 타임별 집계에서 두 과목을 각각 더하고 있었습니다. 화면의 타임별 인원 합 2,090(정답 1,900) · 반 합 82(정답 74)였고, **초과분 190이 진로 수강생 전체와 정확히 일치**해 원인이 확정됐습니다. 구획의 첫 과목에서만 집계하고 나머지는 `shared` 플래그로 건너뛰도록 수정. **검증 기준은 "학생은 한 타임에 한 수업만 들으므로 타임별 인원 합계 = 전체 학생 수"라는 불변식** — 수정 후 샘플 192명으로 과목 추가/학점 변경/삭제/공통과목 해제까지 6단계를 돌려 모든 단계에서 이 값이 맞음을 확인했습니다.
- **부가 분석 — 학기 중 과목 변경 가능성(`analysis/feasibility.js`).** 사용자가 "배정 후 과목을 바꾸면 얼마나 어렵냐"고 물어서 실제 배정 결과 190명 × 13,680가지 조합을 전수 검사했습니다. 다른 학생·분반 배치는 고정하고 한 학생만 움직인다는 전제. **정원 31명이면 변경 불가 14.1%, 32명이면 3.6%** — 정원 한 명 차이가 결정적이고, 3.6%는 정원을 아무리 늘려도 안 사라지는 구조적 한계(공통과목 타임 충돌)입니다. 분반이 1~2개뿐인 과목이 어렵고(인문학과 윤리 33.5%), 6분반인 사회문제탐구는 0%. **처음엔 정원 29로 계산해 70%가 나왔는데, 실제 배정 결과에 이미 31명짜리 분반이 5개 있어 기준이 틀렸던 것**이었습니다 — 설정값이 아니라 데이터에서 실제 최대 인원을 먼저 확인해야 했습니다.
- **개인정보:** 이 저장소의 GitHub 원격이 **공개**임을 확인하고(`api.github.com` 200), 실제 학생 학번이 든 `analysis/assignment.csv`는 커밋하지 않고 폴더 `.gitignore`로 막았습니다(로컬에는 남김). 프로토타입이 쓰는 `public/sample.tsv`는 제가 생성한 합성 데이터(`학생1`~`학생192`)라 안전합니다. **다만 기존 `samples/sample3.xlsx`에 실제 학생 이름·학번이 들어 있고 이미 공개 저장소에 커밋돼 있습니다** — 사용자 판단이 필요해 이번엔 건드리지 않고 AGENTS.md 상단에 확인 요청으로 남겼습니다.
- 이식은 아직 안 했습니다. 계획(순수 로직 → `lib/domain`+`lib/scheduling` 분리, 화면은 Tailwind로 재작성, `enrollment-helper` 4번째 탭 편입 제안)과 미확인 결정 3가지는 `prototypes/time-allocation/README.md` 8·9절에 정리돼 있습니다.

### 2026-09-06 (2)

**쌤스 헬퍼 로그인에 "자동 로그인" 체크박스 + 보강원 트레이 결강 교사별 그룹핑:**
- **자동 로그인**: 사용자가 "로그인 한 컴퓨터는 다시 로그인 안 해도 되게" 요청. 확인해보니 better-auth 기본값(세션 `expiresIn` 7일, `rememberMe` 기본 `true`)으로 이미 절반은 되고 있었음 — 진짜 부족한 건 "7일 넘게 안 쓰면 다시 로그인해야 하는" 것뿐. `src/lib/auth.ts`에 `session.expiresIn`을 60일 → (사용자가 "60일이 최대냐"고 물어서 상한 없음을 확인한 뒤) **180일**로, `updateAge`는 14일로 설정. 로그인 화면(`login/page.tsx`)에 체크박스를 추가해 `signIn.email`/`login-id` 라우트의 `rememberMe`를 사용자가 직접 고르게 함(공용 PC는 끄면 브라우저 세션 쿠키로만 발급). 라벨은 "이 기기 자동 로그인 유지(최대 180일, 공용 PC는 꺼주세요)"로 시작했다가 사용자가 "그냥 자동 로그인이라는 말만 붙여"라고 해서 "자동 로그인"으로 줄임. `login-id/route.ts`도 body의 `rememberMe`(기본 `true`)를 `auth.api.signInEmail`에 그대로 전달하도록 수정.
- **보강원 트레이 그룹핑**: 사용자가 겹침 판정 질문("신경진 자리가 왜 겹침으로 막히냐") 답변 중 "여러 명 한 번에 작성하니 파악이 잘 안 된다"고 지적. 원인은 `MakeupTray.tsx`가 담긴 항목을 결강 교사 구분 없이 순서대로만 나열해서(각 줄의 작은 "OO 결강" 글자로만 구분해야 함). Artifact로 지금/개선안 비교 목업(실제 앱 톤·서식 그대로 재현)을 먼저 보여주고 사용자가 A안(교사별 그룹) + B안(상단 요약 칩)을 같이 승인. `distinctTeachers`로 그룹핑(`groupedEntries`), 항목 카드를 `renderEntryCard(entry, showAbsentTeacher)`로 추출해 그룹 안에서는 반복되는 "OO 결강" 텍스트를 생략. 교사가 1명뿐이면 그룹 헤더 없이 기존 플랫 렌더링 그대로 유지(불필요한 헤더 방지). 검증: 로그인 게이트 밖 임시 페이지(`src/app/dev-preview-tray/page.tsx`, 4건짜리 fixture — 2026-08-21 보강원 검증 때와 같은 패턴)로 브라우저에서 실제 렌더 확인 후 삭제.
- 둘 다 `tsc --noEmit` 클린.
