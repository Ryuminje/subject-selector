...
import { Upload, FileText, Settings, Download, CheckCircle2, ChevronRight, ChevronDown, Trash2, File as FileIcon, Save, FolderOpen, GitBranch, Plus, Users, RotateCcw, Menu } from "lucide-react";
...
const SearchableSelect = ({ options, value, onChange, placeholder = "검색..." }: { options: string[], value: string, onChange: (v: string) => void, placeholder?: string }) => {
        ...
        <header className="flex-none px-10 py-5 border-b border-slate-800/30 bg-slate-950/40 backdrop-blur-sm flex flex-col gap-4">
          <div className="flex items-center justify-between w-full">
            <h1 className="text-2xl font-extrabold tracking-tight text-white">
              {activeSidebarTab === "survey" ? "명신고등학교 수강신청(본조사) 데이터 정리" : "명신고등학교 선택과목 변경 시스템"}
            </h1>
                      ...
                      <div className="grid grid-cols-5 gap-8">
                        ...
                        <div className="col-span-3 space-y-6">
                          <div className="bg-slate-950/40 border border-slate-800/50 rounded-2xl p-6 h-full flex flex-col">
                            ...
                            <p className="text-sm text-slate-300 mb-6">
                              선택하신 학년이 기준이 됩니다. 수강신청(본조사)와 독립적으로 검증에 사용될 교육과정 엑셀 파일을 업로드해 주세요.
                            </p>
                      ...
                      {parsedSampleData[changeActiveGrade] && parsedSampleData[changeActiveGrade].length > 0 ? (
                        <div className="bg-slate-800/30 rounded-2xl p-8 border border-emerald-500/30 flex flex-col items-center justify-center min-h-[300px] text-center">
                          ...
                          <p className="text-slate-300 mb-4 text-center max-w-md">
                            과목명이 열 헤더로 지정되어 있고, 셀 값으로 A, B, C, D 등의 선택 그룹이 명시된 수강신청(본조사) 결과 파일을 업로드해 주세요.
                          </p>
                      ...
                      <div className="flex justify-between items-center mb-2">
                        <div>
                          ...
                          <p className="text-slate-300 text-sm mt-1 mb-4 ml-8">
                            * 위계성 검사는 '수강신청(본조사)' 탭의 2단계에서 설정한 위계 규칙을 공유하여 그대로 적용합니다.
                          </p>
                        </div>
