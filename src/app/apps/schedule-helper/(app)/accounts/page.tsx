  ...
  if (!isAdmin) {
    return (
      <main className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-lg font-semibold text-stone-600">관리자만 접근할 수 있는 페이지입니다.</p>
        <Link href="/apps/schedule-helper" className="inline-block mt-6 text-swap font-semibold hover:underline">
          돌아가기
        </Link>
      </main>
    );
  ...
  return (
    <main className="max-w-4xl mx-auto px-2 md:px-6 py-6 w-full">
      <Link
        ...
        className="inline-flex items-center gap-1.5 mb-4 px-3 py-1.5 bg-white/80 hover:bg-white text-swap text-sm font-medium rounded-[10px] border border-swap/20 transition-colors"
      >
        <ArrowLeft className="w-4 h-4" />
        수업교체 도우미로 돌아가기
      </Link>

      <div className="bg-swap p-6 md:p-8 rounded-[14px] text-white mb-8">
        <h1 className="font-display text-2xl md:text-3xl mb-2 flex items-center gap-2">
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6 mb-6">
        ...
        {loadingMembers ? (
          <div className="flex justify-center py-8 text-swap">
            <Loader2 className="w-6 h-6 animate-spin" />
          </div>
        ) : !members?.length ? (
          <p className="text-sm text-stone-400 text-center py-6">가입된 인원이 없습니다.</p>
        ) : (
          <div className="space-y-2">
            {members.map((m) => {
              ...
              return (
                <div
                  key={m.id}
                  className="flex items-center justify-between gap-2 p-3.5 bg-stone-50 border border-stone-200 rounded-[10px]"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-stone-800">{m.name}</span>
                      {isSelf && <span className="text-xs text-stone-400">(나)</span>}
                      <span
                        className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                          m.role === "ADMIN" ? "bg-swap/10 text-swap" : "bg-stone-200 text-stone-600"
                        }`}
                      >
                        {m.role === "ADMIN" ? "관리자" : "교사"}
                      </span>
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-white border border-stone-200 text-stone-500">
                        {m.loginId ? "아이디 로그인" : "이메일"}
                      </span>
                    </div>
                    <p className="text-sm text-stone-500 truncate">{m.loginId ?? m.email}</p>
                  </div>
                  {!isSelf && (
                    <button
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6 mb-6">
        <h2 className="font-display text-lg text-swap mb-1">학교 코드</h2>
        <p className="text-sm text-stone-500 mb-4">
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6 mb-6">
        ...
        <p className="text-sm text-stone-500 mb-4">아이디는 한글/영문/숫자/._- 를 사용해 2~30자로 입력하세요.</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          <input
            ...
            placeholder="이름"
            className="px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-[10px] text-sm focus:outline-none focus:ring-2 focus:ring-swap/30 focus:border-swap transition-colors"
          />
          <input
            type="text"
            value={loginId}
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-swap hover:opacity-90 disabled:opacity-50 text-white text-sm font-bold rounded-[10px] transition-opacity"
        >
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          ...
          <input
            ...
            className="sm:w-56 px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-[10px] text-sm focus:outline-none focus:ring-2 focus:ring-swap/30 focus:border-swap transition-colors"
          />
          <button
            onClick={handleBulkUpload}
            disabled={bulkUploading || !bulkFile || !bulkPassword}
            className="px-5 py-2.5 bg-swap hover:opacity-90 disabled:opacity-50 text-white font-semibold rounded-[10px] transition-opacity flex items-center justify-center gap-2 shrink-0"
          >
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6">
        <h2 className="font-display text-lg text-swap mb-4">아이디 로그인 계정 목록</h2>
        {loadingAccounts ? (
          <div className="flex justify-center py-8 text-swap">
            <Loader2 className="w-6 h-6 animate-spin" />
          </div>
        ) : !accounts?.length ? (
          <p className="text-sm text-stone-400 text-center py-6">아직 만든 아이디 계정이 없습니다.</p>
        ) : (
          <div className="space-y-2">
            {accounts.map((a) => (
              <div key={a.id} className="p-3.5 bg-stone-50 border border-stone-200 rounded-[10px]">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <span className="font-bold text-stone-800">{a.name}</span>{" "}
                    <span className="text-sm text-stone-500">· {a.loginId}</span>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="inline-flex items-center gap-1 text-xs px-3 py-1.5 border border-stone-200 rounded-lg text-stone-600 hover:bg-stone-100 transition-colors shrink-0"
                  >
                ...
                {resetTargetId === a.id && (
                  <div className="mt-3 flex flex-col sm:flex-row gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-white border border-stone-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-swap/30 focus:border-swap"
                    />
                    <button
                      onClick={() => handleResetPassword(a.id)}
                      disabled={resetting}
                      className="px-4 py-2 bg-swap hover:opacity-90 disabled:opacity-60 text-white text-sm font-bold rounded-lg transition-opacity shrink-0"
                    >
