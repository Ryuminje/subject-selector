...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 협의회 프리셋·교체 작업 세트는 User에 @relation을 걸지 않아(better-auth CLI가 User
  // 블록을 다시 쓰기 때문) cascade가 걸리지 않습니다. 계정을 지울 때 여기서 직접 지워야
  // 고아 행이 남지 않습니다.
  await prisma.meetingPreset.deleteMany({ where: { userId: id } });
  await prisma.makeupBatch.deleteMany({ where: { userId: id } });
