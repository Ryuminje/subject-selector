    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {step6Data.length === 0 ? (
        <div className="bg-white/70 border border-stone-200 rounded-2xl p-8 text-center">
          <p className="text-stone-600">선택하신 학년의 데이터가 아직 없습니다.</p>
        </div>
      ) : (
          ...
          <div className="overflow-auto max-h-[650px] relative">
            <table className="w-full text-sm text-left text-stone-600 border-collapse">
              <thead className="text-xs text-stone-600 uppercase bg-white border-b border-stone-200">
                <tr>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 left-0 z-40 bg-white min-w-[50px] max-w-[50px] border-r border-b border-stone-200 text-center align-middle">순번</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 left-[50px] z-40 bg-white min-w-[80px] max-w-[80px] border-r border-b border-stone-200 text-center align-middle">학번</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 left-[130px] z-40 bg-white min-w-[80px] max-w-[80px] border-r border-b border-stone-200 text-center shadow-[2px_0_5px_rgba(0,0,0,0.3)] align-middle">이름</th>
                  <th rowSpan={2} className="px-2 py-2.5 text-center whitespace-nowrap sticky top-0 z-10 bg-white border-r border-b border-stone-200 align-middle">과거 이수 과목</th>
                  <th colSpan={timeSlots[changeActiveGrade]?.length || 1} className="px-2 py-1 text-center whitespace-nowrap sticky top-0 z-10 bg-white border-r border-b border-stone-200 text-[11px] text-stone-400">2학기 과목</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 z-10 bg-white border-l border-b border-stone-200 text-center align-middle">기초과목</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 z-10 bg-white border-b border-stone-200 text-center align-middle">사회</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 z-10 bg-white border-b border-stone-200 text-center align-middle">과학</th>
                  <th rowSpan={2} className="px-2 py-2.5 whitespace-nowrap sticky top-0 z-10 bg-white border-b border-stone-200 align-middle">비고(중복/위계)</th>
                </tr>
                <tr>
                  {timeSlots[changeActiveGrade]?.map(slot => (
                    <th key={slot} className="px-2 py-1.5 text-center whitespace-nowrap sticky top-[25px] z-10 bg-white border-r border-b border-stone-200 text-amber-600">{slot}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(showOnlyApplicants ? step6Data.filter(row => adjustmentLog[row.id]?.some(l => l.status === 'success' && l.source === 'applicant')) : step6Data).map((row, idx) => {
                  ...
                  <tr key={idx} className="group border-b border-stone-200 hover:bg-stone-100">
                    ...
                    <td className={`px-2 py-2.5 font-medium whitespace-nowrap sticky left-[50px] z-20 min-w-[80px] max-w-[80px] border-r border-stone-200 text-center ${highlightClass} ${hoverClass}`}>{row.id}</td>
                    <td className={`px-2 py-2.5 font-medium whitespace-nowrap sticky left-[130px] z-20 min-w-[80px] max-w-[80px] border-r border-stone-200 text-center shadow-[2px_0_5px_rgba(0,0,0,0.3)] ${nameHighlightClass} ${hoverClass}`}>{row.name}</td>
                    <td className="px-2 py-2.5 border-r border-stone-200 max-w-[250px] text-xs leading-relaxed break-words" title={row.completedBefore.join(", ")}>
                      {row.completedBefore.length > 0 ? row.completedBefore.join(", ") : <span className="text-stone-500">-</span>}
                    </td>
                    {timeSlots[changeActiveGrade]?.map(slot => {
