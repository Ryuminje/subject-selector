      ...
      <div className="flex items-center justify-between">
        ...
        {!expanded && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-stone-600 hover:bg-stone-500 text-white text-sm font-semibold rounded-[10px] transition-colors"
          >
            관리하기
          </button>
        )}
        ...
        <div>
          <div className="flex gap-2 mb-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-cert hover:opacity-90 disabled:opacity-60 text-white text-sm font-bold rounded-[10px] transition-opacity shrink-0"
            >
          ...
          {entries.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {entries.map((e) => (
                <span
                  key={e.id}
                  className="inline-flex items-center gap-1.5 bg-stone-50 border border-stone-200 text-stone-700 pl-3 pr-1.5 py-1.5 rounded-full text-xs font-semibold"
                >
                  {e.name}
                  <button
