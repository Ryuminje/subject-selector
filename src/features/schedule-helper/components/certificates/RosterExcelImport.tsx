...
export default function RosterExcelImport({
  ...
  return (
    <div className="border-2 border-dashed border-cert/30 rounded-[10px] p-4 bg-cert/5 mb-4">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-bold text-cert flex items-center gap-1.5">
          <FileSpreadsheet className="w-4 h-4" />
          {mode === "create" ? "엑셀로 새 명단 만들기" : "엑셀에서 이름 이어 붙이기"}
        </h3>
        <button
          onClick={onCancel}
          className="p-1 rounded-lg text-stone-400 hover:text-stone-600 hover:bg-white transition-colors"
          aria-label="닫기"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 text-xs font-semibold text-cert hover:opacity-80 hover:underline mb-3"
      >
      ...
      <input
        ...
        className="w-full text-sm text-stone-600 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-cert/15 file:text-cert file:font-semibold hover:file:bg-cert/20 file:cursor-pointer cursor-pointer"
      />

      {parsing && (
        <p className="mt-3 text-sm text-cert flex items-center gap-1.5">
      ...
      {sheets && sheet && (
        <div className="mt-4 space-y-3">
          {sheets.length > 1 && (
            <div>
              <div className="text-xs font-semibold text-stone-500 mb-1.5">시트</div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
                      i === sheetIndex
                        ...
                        : "bg-white border-stone-200 text-stone-600 hover:border-cert/40"
                    }`}
                  >
                    {s.name}
                  </button>
          ...
          <div>
            <div className="text-xs font-semibold text-stone-500 mb-1.5">이름이 있는 열</div>
            <div className="flex flex-wrap gap-1.5">
              {sheet.columns.map((c) => {
          ...
          <label className="flex items-center gap-2 text-xs text-stone-600 cursor-pointer">
            <input
              ...
              className="w-4 h-4 accent-cert"
            />
            첫 줄은 머리글이라 건너뜁니다
            {column?.cells[0] && <span className="text-stone-400">(첫 줄: {column.cells[0]})</span>}
          </label>

          <div className="bg-white border border-stone-200 rounded-[10px] p-3">
          ...
          <div className="flex justify-end gap-2">
            <button
              onClick={onCancel}
              className="px-4 py-2 text-sm font-semibold text-stone-500 hover:bg-white rounded-[10px] transition-colors"
            >
              취소
            </button>
            <button
              ...
              className="px-4 py-2 bg-cert hover:opacity-90 disabled:opacity-50 text-white text-sm font-bold rounded-[10px] transition-opacity"
            >
