    ...
    <div className="space-y-6">
      {isAdmin && (
        <div className="bg-white rounded-[14px] border border-stone-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-bold text-stone-700 mb-1.5 block">성명으로 조회 (정확히 입력)</label>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-stone-600 hover:bg-stone-500 text-white text-sm font-semibold rounded-[10px] transition-colors shrink-0"
                >
            ...
            <div>
              <label className="text-sm font-bold text-stone-700 mb-1.5 block">연수 제목으로 조회 (포함 검색)</label>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={titleQuery}
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6">
        <h2 className="font-display text-lg text-cert mb-4 flex items-center gap-2">
          ...
          {rows && <span className="text-sm font-medium text-stone-500 ml-1">({rows.length}건)</span>}
        </h2>
        {!isAdmin && (
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-stone-400">
            ...
            <p className="font-semibold text-stone-500">{isAdmin ? "검색 결과가 없습니다." : "제출한 이수증이 없습니다."}</p>
          </div>
        ) : (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {rows.map((row) => (
                <div
                  key={row.id}
                  className="relative p-4 bg-stone-50 hover:bg-cert/8 border border-stone-200 hover:border-cert/40 rounded-[10px] transition-colors group"
                >
                  <a
                  ...
                  >
                    <div className="font-bold text-stone-800 mb-1">{row.trainingTitle}</div>
                  ...
                  {(isAdmin || row.teacherName === myName) && (
                    <button
                      ...
                      title="삭제"
                      className="absolute top-3 right-3 p-1.5 rounded-full text-stone-400 hover:bg-rose-100 hover:text-rose-600 disabled:opacity-60 transition-colors"
                    >
                      {deletingId === row.id ? (
