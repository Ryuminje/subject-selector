  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-[14px] border border-stone-200 p-6">
        ...
        <label className="text-sm font-bold text-stone-700 mb-1.5 block">참여 명단</label>
        <select
          value={selectedPresetId}
          onChange={(e) => setSelectedPresetId(e.target.value)}
          className="w-full mb-4 px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-[10px] text-sm focus:outline-none focus:ring-2 focus:ring-cert/30 focus:border-cert transition-colors"
        >
        ...
        <p className="text-xs text-stone-400 -mt-2.5 mb-4">
          ...
          고르면 선택한 모든 연수에 그 명단이 동일하게 적용됩니다. 프리셋은 &quot;공통 설정&quot; 탭에서 만들 수
          있습니다.
        </p>
      ...
      <div className="bg-white rounded-[14px] border border-stone-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="text-xs px-3 py-1.5 border border-stone-200 rounded-lg text-stone-500 hover:bg-stone-50 transition-colors"
          >
            새로고침
          </button>
        </div>
        ...
        {loadingSessions ? (
          ...
          <p className="text-sm text-stone-400 text-center py-8">이전 세션이 없습니다.</p>
        ) : (
          <div className="space-y-2">
            {pastSessions.map((s) => (
              <button
                ...
                className="w-full text-left p-3.5 bg-stone-50 hover:bg-cert/8 border border-stone-200 hover:border-cert/40 rounded-[10px] transition-colors"
              >
                <div className="flex items-center gap-2 font-bold text-stone-800 mb-1">
                  ...
                  {s.isGroup && (
                    <span className="text-[11px] bg-cert/8 text-cert px-2 py-0.5 rounded-full font-semibold">
                      복수연수
                    </span>
                  )}
                  {s.rosterPresetName && (
