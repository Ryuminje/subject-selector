/**
 * 시험지 봉투 표지(A5) 인쇄용 데이터 넘기기.
 *
 * 표지 내용은 `.xls` 메일머지와 **같은 데이터**입니다(`buildEnvelopeRows`) — 한쪽만 고쳐
 ...
 * 용지 배치.
 *  - `a5`: A5 가로 한 장에 표지 하나.
 *  - `a4-2up`: A4 세로 한 장에 표지 둘(위·아래). A5 가로의 세로 길이가 148mm라
 *    두 장이 297mm에 정확히 들어갑니다.
 */
export type EnvelopeLayout = 'a5' | 'a4-2up';

/**
 * 표지를 새 탭에서 엽니다. 만들 줄이 없으면 아무것도 하지 않고 `false`입니다.
